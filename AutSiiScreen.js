...
// create a component
const AutSiiScreen = ({route, navigation}) => {
    ...
    React.useEffect(() => {
      ...
      fetchCertSii = async (x) => {
        await fetch(`${ServidorExport}/partner/TraerCertSii.php`, {
           method: 'POST',
           headers: {
          ...
          return (
            <View>
              ...
              {dataSource.map((data, i) => {
                return (
                  ...
                  >
                      <Image 
                        source={{ uri: `https://www.convey.cl/partner/certsii/${data.certificado}` }} 
                        style={{ width: '100%', aspectRatio: 4 / 3, borderRadius: 25, transform: [{ rotate: '90deg' }], }} 
                        resizeMode="contain" 
                        
                      />
                    <View style={{flexDirection:'row', gap:10, justifyContent:'flex-end', alignItems:'flex-end', marginTop:15,}}>
                        <TouchableOpacity onPress={() => {navigation.navigate('AddAutSii',{IdCertificado: data.idcertsii, Certificado: data.certificado,})}}  style={{ padding: 0, marginBottom: 10, alignSelf:'flex-end', marginRight:15,}}>
                            <FontAwesome name="pencil" style={styles.arrowBack} />
                        </TouchableOpacity>

                        <TouchableOpacity style={{ padding: 0, marginBottom: 10, alignSelf:'flex-end', marginRight:15,}} onPress={() => DeleteAsk(data.idcertsii)}>
            ...
            return(
               <View style={styles.ctnnoparse}>
                        <TouchableOpacity onPress={() => {navigation.navigate('AddAutSii',{IdCertificado: '', Certificado: ''})}} style={{width:'100%', backgroundColor:'#ededed', borderRadius:10, paddingVertical:25, justifyContent:'center', alignItems:'center'}}>
                            <FontAwesome name="plus" style={{color:'#757575', fontSize:fontWidth/12, marginBottom:15}} />
                            <Text style={styles.txtRenderElements}>Agregar certificado</Text>
                        </TouchableOpacity>
                </View>
              )
        }
      ...
      DeleteAsk = (id) =>{
        ModalOptions('Alerta', '¿Eliminar?', '¿Estás seguro quieres eliminar el certificado?', () => DeleteCert(id));
      }
      DeleteCert = (id) =>{
        console.log(id)
        console.log(user)
        setActivityShow(prev => true)
        fetch(`${ServidorExport}/partner/BorrarCedula.php`, {
          method: 'POST',
          headers: {
            'Accept': 'application/json',
            'Content-Type': 'application/json',
              ...
              response.json()
                .then(responser => {
                  ...
                  if (responser === 'exito') {
                    fetchLicencia(user)
                    setActivityShow(prev => false)
                    ModalOptions('Exito', 'Eliminado', 'El certificado se eliminó con éxito', 'close' );
                  }else if(responser === 'existe'){
                    ModalOptions('Error', 'Error', 'Debes tener al menos un certificado','close');
                    setActivityShow(prev => false)
                  }
                  else{
        ...
        <View style={{ flex: 1, backgroundColor:'#fff' }}>
            ...
            <ScrollView style={{flex:1}} keyboardShouldPersistTaps='always'>
              ...
              <View style={styles.header}>
               ...
               <Text style={styles.TextHeader}>Autorización de proveedor de facturación de servicio de impuestos internos</Text>
