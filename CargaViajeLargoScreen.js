...
// create a component
const CargaViajeLargoScreen = ({route, navigation}) => {
  const socket = getSocket();
    ...
    React.useEffect(() => {
        const unsubscribe = navigation.addListener('focus', async () => {
            var userToken = await AsyncStorage.getItem('Key_27');
            VistoCamionero()
            fetchCargaViajeLargo(userToken)
            chatExist(userToken)
            //connectScoket()
    ...
    fetchCargaViajeLargo = async (x) => {
      console.log('ixx')
      console.log(idCarga)
        await fetch(`${ServidorExport}/partner/viajelargo/TraerCargaUnicaViajeLargo.php`, {
           method: 'POST',
           headers: {
      ...
      const VistoCamionero = () =>{
        ...
        fetch(`${ServidorExport}/partner/viajelargo/MarcarVistoPago.php`, {
          method: 'POST',
          headers: {
              Accept: "application/json",
      ...
      const cargarViajeLargo = () =>{
        ...
        if(kilosSelected == true){
            kilosSend = 'si'
        }else{
            kilosSend = 'no'
        }

        if(bultosSelected == true){
            bultosSend = 'si'
        }else{
            bultosSend = 'no'
        }
        ...
        data.append('kilosacept', kilosSend);
        data.append('bultosacept', bultosSend);
        ...
        fetch(`${ServidorExport}/partner/viajelargo/CargarViajeLargo.php`, {
          method: 'POST',
          headers: {
              Accept: "application/json",
              "Content-Type": "multipart/form-data"
      ...
      const EntregarCarga = async () =>{
        ...
        fetch(`${ServidorExport}/partner/viajelargo/EntregarViajeLargo.php`, {
          method: 'POST',
          headers: {
              Accept: "application/json",
              "Content-Type": "multipart/form-data"
                ...
                if(data.estado == 'buscando'){
                    return(
                        <View key={i}>
                                    ...
                                    <TouchableOpacity onPress={() => setKilosSelected(!kilosSelected)} style={{width:'100%', flexDirection:'row', alignItems:'center', marginBottom:15, marginTop:15,}}>
                                        <View style={{width:fontWidth/18, height: fontWidth/18, marginRight:10, borderWidth:2, borderColor:'#07E607', borderRadius:100, justifyContent:'center', alignItems:'center'}}>
                                            <View style={{width:fontWidth/28, height: fontWidth/28, borderRadius:100, backgroundColor: kilosSelected == true ? '#07E607' : '#fff'}}/>
                                        </View>
                                        <Text style={{ color: '#262626', fontFamily:'Poppins_700Bold', fontSize: fontWidth / 28}}>Kilos</Text>
                                    </TouchableOpacity>
                    

                                    <TouchableOpacity onPress={() => setBultosSelected(!bultosSelected)} style={{width:'100%', flexDirection:'row', alignItems:'center', marginBottom:15,}}>
                                        <View style={{width:fontWidth/18, height: fontWidth/18, marginRight:10, borderWidth:2, borderColor:'#07E607', borderRadius:100, justifyContent:'center', alignItems:'center'}}>
                                            <View style={{width:fontWidth/28, height: fontWidth/28, borderRadius:100, backgroundColor: bultosSelected == true ? '#07E607' : '#fff'}}/>
                                        </View>
                                        <Text style={{ color: '#262626', fontFamily:'Poppins_700Bold', fontSize: fontWidth / 28}}>Bultos</Text>
                                    </TouchableOpacity>
                                    ...
                                    <LinearGradient style={{width:'100%', height:50, borderRadius:10, marginTop:25, justifyContent:'center', alignItems:'center'}} colors={['#E6F14B', '#E6F14B', '#C3FE70']} start={{ x: 1, y: 0 }} end={{ x: 0, y: 0 }}>
                                            ...
                                            <TouchableOpacity onPress={() => cargarViajeLargo()} style={{width:'100%', height:'100%', justifyContent:'center', alignItems:'center'}} activeOpacity={1} >
...
export default CargaViajeLargoScreen;
