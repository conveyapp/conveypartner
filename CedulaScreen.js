...
// create a component
const CedulaScreen = ({route, navigation}) => {
    ...
    React.useEffect(() => {
      ...
      fetchCedula = async (x) => {
        await fetch(`${ServidorExport}/partner/TraerCedula.php`, {
           method: 'POST',
           headers: {
          ...
          return (
            <View>
              ...
              {dataSource.map((data, i) => {
                return (
                  ...
                  >
                      <Image 
                        source={{ uri: `https://www.convey.cl/partner/carnetimgs/${data.anverso}` }} 
                    ...
                    <View style={{flexDirection:'row', gap:10, justifyContent:'flex-end', alignItems:'flex-end', marginTop:15,}}>
                        <TouchableOpacity onPress={() => {navigation.navigate('AddCedula',{IdCedula: data.idcedula, ImgAnverso:data.anverso, ImgReverso:data.reverso,})}}  style={{ padding: 0, marginBottom: 10, alignSelf:'flex-end', marginRight:15,}}>
                            <FontAwesome name="pencil" style={styles.arrowBack} />
                        </TouchableOpacity>

                        <TouchableOpacity style={{ padding: 0, marginBottom: 10, alignSelf:'flex-end', marginRight:15,}} onPress={() => DeleteAsk(data.idcedula)}>
            ...
            return(
               <View style={styles.ctnnoparse}>
                        <TouchableOpacity onPress={() => {navigation.navigate('AddCedula',{IdCedula: '', ImgAnverso:'', ImgReverso:'',})}} style={{width:'100%', backgroundColor:'#ededed', borderRadius:10, paddingVertical:25, justifyContent:'center', alignItems:'center'}}>
                            <FontAwesome name="plus" style={{color:'#757575', fontSize:fontWidth/12, marginBottom:15}} />
                            <Text style={styles.txtRenderElements}>Agregar cédula de identidad</Text>
                        </TouchableOpacity>
                </View>
              )
        }
      ...
      DeleteAsk = (id) =>{
        ModalOptions('Alerta', '¿Eliminar?', '¿Estás seguro quieres eliminar la cédula?', () => DeleteCedula(id));
      }
      DeleteCedula = (id) =>{
        console.log(id)
        console.log(user)
              ...
              response.json()
                .then(responser => {
                  ...
                  if (responser === 'exito') {
                    fetchLicencia(user)
                    setActivityShow(prev => false)
                    ModalOptions('Exito', 'Eliminada', 'La cédula se eliminó con éxito', 'close' );
                  }else if(responser === 'existe'){
                    ModalOptions('Error', 'Error', 'Debes tener al menos una cédula','close');
                    setActivityShow(prev => false)
                  }
                  else{
        ...
        <View style={{ flex: 1, backgroundColor:'#fff' }}>
            ...
            <ScrollView style={{flex:1}} keyboardShouldPersistTaps='always'>
              ...
              <View style={styles.header}>
               ...
               <Text style={styles.TextHeader}>Cédula de identidad</Text>
