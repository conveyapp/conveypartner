...
const CrearViajeLargoScreen = ({ route, navigation }) => {
  ...
  crearAndroidViaje = async () => {
    ...
    } else if (fechaFin.diff(fechaIni, 'days') > 31) {
      ModalOptions('Error', 'Duración no permitida', 'El viaje no puede durar más de un mes', 'close');
    } else {
      // Realizar la petición al servidor
      fetch(`${ServidorExport}/partner/viajelargo/CrearViajeLargo.php`, {
        method: 'POST',
        headers: {
  ...
  crearIosViaje = async () => {
    ...
    if (desde === '') {
      ...
      ModalOptions('Error', 'Fecha de llegada inválida', 'La fecha de llegada no puede ser anterior a la de inicio', 'close');
    } else if (fechaFinPass.diff(fechaIniPass, 'days') > 31) {
      ModalOptions('Error', 'Duración no permitida', 'El viaje no puede durar más de un mes', 'close');
    } else {
      fetch(`${ServidorExport}/partner/viajelargo/CrearViajeLargo.php`, {
        method: 'POST',
          ...
          >
            ...
            <ScrollView style={{ flex: 1 }} keyboardShouldPersistTaps='always'>
              ...
              <View style={{ flex: 1, width: '100%', marginTop: 2, paddingHorizontal: 15, backgroundColor: 'transparent' }}>
                ...
                {/* <View style={{paddingHorizontal:15, paddingVertical:10, borderRadius:10, backgroundColor:'#93CC49', alignSelf:'flex-start', marginBottom:10,}}>
                ...
                {/* 
                <TouchableOpacity activeOpacity={1} style={styles.ctninputs} onPress={toggleOverlayDos}>
                  <View style={{ width: '80%' }}>
                  <Text style={styles.labels}>¿Hasta dónde llegas?</Text>
                    <TextInput
                      ...
                      onChangeText={hast => setHasta(hast)}
...
export default CrearViajeLargoScreen;
