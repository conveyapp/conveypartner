...
// create a component
const DetalleCalifScreen = ({route, navigation}) => {
    
    const [ActivityShow, setActivityShow] = React.useState('');

    const [IdCalif, setIdCalif] = React.useState(route.params.IdCalif);
    const [Calificacion, setCalificacion] = React.useState(route.params.Calificacion);
    const [MotivoUno, setMotivoUno] = React.useState(route.params.MotivoUno);
    const [MotivoDos, setMotivoDos] = React.useState(route.params.MotivoDos);
    const [MotivoTres, setMotivoTres] = React.useState(route.params.MotivoTres);
    const [Comentario, setComentario] = React.useState(route.params.Comentario);
    ...
    return (
        <View style={{ flex: 1, backgroundColor:'#fff' }}>
            ...
            <ScrollView style={{flex:1}} keyboardShouldPersistTaps='always'>
              ...
              <Text style={{ color: '#262626', fontFamily: 'Poppins_700Bold', fontSize: fontWidth / 28 }}>{IdCalif}</Text>
              <Text style={{ color: '#262626', fontFamily: 'Poppins_700Bold', fontSize: fontWidth / 28 }}>{Comentario}</Text>

              {MotivoUno === '' ? (
                    null
                    ) : (
                    <View>
                        <Text style={{ color: '#262626', fontFamily: 'Poppins_700Bold', fontSize: fontWidth / 28 }}>{MotivoUno}</Text>
                        </View>
              )}
            ...
            {MotivoDos === '' ? (
                    ...
                    <View>
                        <Text style={{ color: '#262626', fontFamily: 'Poppins_700Bold', fontSize: fontWidth / 28 }}>{MotivoDos}</Text>
                        </View>
              )}
            ...
            {MotivoTres === '' ? (
                    ...
                    <View>
                        <Text style={{ color: '#262626', fontFamily: 'Poppins_700Bold', fontSize: fontWidth / 28 }}>{MotivoTres}</Text>
                        </View>
              )}

  
                <View style={{width:'100%', paddingHorizontal:40, marginTop:25}}>
                    {rendStars(Calificacion)}
                </View>
...
export default DetalleCalifScreen;
