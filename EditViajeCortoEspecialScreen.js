...
const EditViajeCortoEspecialScreen = ({ route, navigation }) => {
  ...
  const [fueraciudad, setFueraciudad] = React.useState(false);
  ...
  React.useEffect(() => {
    if (InfoViaje[0].fueraciudad == 'true') {
      setFueraciudad(true)
    }
    if (InfoViaje[0].fueraciudad == 'false') {
      setFueraciudad(false)
    }
  ...
  CrearViaje = async () => {
    ...
    fetch(`${ServidorExport}/partner/EditViajeCortoEspecial.php`, {
      method: 'POST',
      headers: {
      ...
      <View style={styles.containerAdornos}>
        ...
        <ScrollView style={{ flex: 1 }} keyboardShouldPersistTaps='always'>
          ...
          <View style={{ flex: 1, width: '100%', marginTop: 2, paddingHorizontal: 15, backgroundColor: 'transparent' }}>
            ...
            <TouchableOpacity onPress={() => setFueraciudad(!fueraciudad)} style={{ width: '100%', flexDirection: 'row', alignItems: 'center', marginBottom: 15 }}>
              <View style={{ width: fontWidth / 18, height: fontWidth / 18, marginRight: 10, borderWidth: 2, borderColor: '#07E607', borderRadius: 100, justifyContent: 'center', alignItems: 'center' }}>
                <View style={{ width: fontWidth / 28, height: fontWidth / 28, borderRadius: 100, backgroundColor: fueraciudad == true ? '#07E607' : '#fff' }} />
              </View>
              <Text style={{ color: '#262626', fontFamily: 'Poppins_700Bold', fontSize: fontWidth / 28 }}>¿Viaje fuera de la ciudad?</Text>
            </TouchableOpacity>
...
export default EditViajeCortoEspecialScreen;
