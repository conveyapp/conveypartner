...
const EditViajeLargoEspecialScreen = ({ route, navigation }) => {
  ...
  const [desde, setDesde] = React.useState(route.params.InfoViaje[0].desde);
  const [hasta, setHasta] = React.useState(route.params.InfoViaje[0].hasta);


  const [nacimiento, setNacimiento] = React.useState('');

  const [fechaIni, setFechaIni] = React.useState(route.params.InfoViaje[0].fechainicio);
  const [fechaFin, setFechaFin] = React.useState(route.params.InfoViaje[0].fechafin);
  ...
  const [otroContent, setOtroContent] = React.useState(route.params.InfoViaje[0].otro);
  ...
  React.useEffect(() => {
    if (route.params.InfoViaje[0].animales == 'true') {
      setAnimales(prev => true)
    } else {
      setAnimales(prev => false)
    }

    if (route.params.InfoViaje[0].aridos == 'true') {
      setAridos(prev => true)
    } else {
      setAridos(prev => false)
    }

    if (route.params.InfoViaje[0].madera == 'true') {
      setMadera(prev => true)
    } else {
      setMadera(prev => false)
    }

    if (route.params.InfoViaje[0].maquinaria == 'true') {
      setMaquinaria(prev => true)
    } else {
      setMaquinaria(prev => false)
    }

    if (route.params.InfoViaje[0].otro != '') {
      setOtro(prev => true)
      setOtroContent(prev => route.params.InfoViaje[0].otro)
    } else {
      setOtro(prev => false)
    }


    setDia(prev => moment(fechaIni, "DD-MM-YYYY").format("DD"))
    setMes(prev => moment(fechaIni, "DD-MM-YYYY").format("MM"))
    setAno(prev => moment(fechaIni, "DD-MM-YYYY").format("YYYY"))

    setDiaDos(prev => moment(fechaFin, "DD-MM-YYYY").format("DD"))
    setMesDos(prev => moment(fechaFin, "DD-MM-YYYY").format("MM"))
    setAnoDos(prev => moment(fechaFin, "DD-MM-YYYY").format("YYYY"))
  ...
  crearAndroidViaje = async () => {
    ...
    var fechaIni = dia + '-' + mes + '-' + ano
    var fechaFin = diaDos + '-' + mesDos + '-' + anoDos

    var fechaIniPass = moment(fechaIni, "DD-MM-YYYY");
    var fechaFinPass = moment(fechaFin, "DD-MM-YYYY");
    ...
    if (desde == '') {
      ...
      ModalOptions('Error', 'Fecha de Partida inválida', 'Debes elegir un día', 'close');
    } else if (mes == '') {
      ModalOptions('Error', 'Fecha de Partida inválida', 'Debes elegir un mes', 'close');
    } else if (ano == '') {
      ModalOptions('Error', 'Fecha de Partida inválida', 'Debes elegir un año', 'close');
    } else if (diaDos == '') {
      ModalOptions('Error', 'Fecha de llegada inválida', 'Debes elegir un día', 'close');
    } else if (mesDos == '') {
      ModalOptions('Error', 'Fecha de llegada inválida', 'Debes elegir un mes', 'close');
    } else if (anoDos == '') {
      ModalOptions('Error', 'Fecha de llegada inválida', 'Debes elegir un año', 'close');
      ...
      fetch(`${ServidorExport}/partner/EditViajeLargoEspecial.php`, {
        method: 'POST',
        headers: {
          ...
          if (!response.ok || response.status != 200 || response.status != '200') {
            ...
            response.json()
              .then(responser => {
                ...
                if (responser === 'exito') {
                  ModalOptions('Exito', 'Viaje actualizado', 'Editaste el viaje nuevo', backPass);
  ...
  crearIosViaje = async () => {
    ...
    if (desde == '') {
      ...
      ModalOptions('Error', 'Llegada no válida', 'Debes ingresar un lugar de llegada', 'close');
    } else if (fechaIni == 'Buscar fecha') {
      ModalOptions('Error', 'Fecha de inicio inválida', 'Debes elegir una fecha de partida', 'close');
    } else if (fechaFin == 'Buscar fecha') {
      ModalOptions('Error', 'Fecha de llegada inválida', 'Debes elegir una fecha de llegada', 'close');
    } else if (fechaIniPass.isBefore(moment())) {
      ModalOptions('Error', 'Fecha de inicio inválida', 'La fecha de inicio ya pasó', 'close');
    } else if (fechaFinPass.isBefore(moment())) {
      ModalOptions('Error', 'Fecha de llegada inválida', 'La fecha de llegada ya pasó', 'close');
  ...
  fetchPlaces = async (x) => {
    ...
    await fetch(`https://maps.googleapis.com/maps/api/place/textsearch/json?query=${lugarABuscar}&key=${apiKey}`, {
      method: 'GET',
    })
      .then((response) => {
          ...
          >
            ...
            <ScrollView style={{ flex: 1 }} keyboardShouldPersistTaps='always'>
              ...
              <View style={{ flex: 1, width: '100%', marginTop: 2, paddingHorizontal: 15, backgroundColor: 'transparent' }}>
                <Text style={[styles.TextHeader, { marginBottom: 5, }]}>Editar viaje</Text>
                ...
                <TouchableOpacity style={styles.ctninputs} activeOpacity={1} onPress={toggleOverlayDos}>
                  <Text style={[styles.labels, { marginLeft: 0 }]}>¿Hasta dónde partes?</Text>
...
export default EditViajeLargoEspecialScreen;
