...
const EditViajeLargoScreen = ({ route, navigation }) => {
  ...
  crearAndroidViaje = async () => {
    ...
    if (desde == '') {
      ...
      ModalOptions('Error', 'Fecha de Partida inválida', 'Debes elegir una fecha completa', 'close');
    } else if (diaDos == '' || mesDos == '' || anoDos == '') {
      ModalOptions('Error', 'Fecha de llegada inválida', 'Debes elegir una fecha completa', 'close');
    } else if (fechaIniPass.isBefore(moment().subtract(1, 'days'))) {
      ModalOptions('Error', 'Fecha de inicio inválida', 'La fecha de inicio no puede ser anterior a hoy', 'close');
    } else if (fechaFinPass.isBefore(fechaIniPass)) {
      ModalOptions('Error', 'Fecha de llegada inválida', 'La fecha de llegada no puede ser anterior a la fecha de inicio', 'close');
    } else if (moment().diff(fechaIniPass, 'days') > 7) {
      ModalOptions('Error', 'Fecha de inicio inválida', 'La fecha de inicio no puede ser mayor a una semana', 'close');
    } else if (moment().diff(fechaFinPass, 'days') > 7) {
      ModalOptions('Error', 'Fecha de llegada inválida', 'La fecha de llegada no puede ser mayor a una semana', 'close');
    } else {
      fetch(`${ServidorExport}/partner/EditViajeLargo.php`, {
        method: 'POST',
        headers: {
        ...
        .then((response) => {
          if (!response.ok) {
            ...
            response.json()
              .then(responser => {
                if (responser === 'exito') {
                  ModalOptions('Éxito', 'Viaje actualizado', 'Editaste el viaje correctamente', backPass);
  ...
  crearIosViaje = async () => {
    ...
    if (desde == '') {
      ...
      ModalOptions('Error', 'Llegada no válida', 'Debes ingresar un lugar de llegada', 'close');
    } else if (fechaIni == 'Buscar fecha' || fechaFin == 'Buscar fecha') {
      ModalOptions('Error', 'Fecha inválida', 'Debes elegir fechas completas', 'close');
    } else if (fechaIniPass.isBefore(moment().subtract(1, 'days'))) {
      ModalOptions('Error', 'Fecha de inicio inválida', 'La fecha de inicio no puede ser anterior a hoy', 'close');
    } else if (fechaFinPass.isBefore(fechaIniPass)) {
      ModalOptions('Error', 'Fecha de llegada inválida', 'La fecha de llegada no puede ser anterior a la fecha de inicio', 'close');
        ...
        .then((response) => {
          if (!response.ok) {
            ...
            response.json()
              .then(responser => {
                if (responser === 'exito') {
                  ModalOptions('Exito', 'Viaje actualizado', 'Editaste el viaje correctamente', backPass);
                } else if (responser === 'existe') {
                  ModalOptions('Error', 'Ya tienes un viaje creado', 'No puedes tener más de un viaje en curso', 'close');
                } else {
                  ModalOptions('Error', 'Error de conexión', 'Algo falló, intenta nuevamente', 'close');
...
export default EditViajeLargoScreen;
