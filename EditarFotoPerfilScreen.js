...
// create a component
const EditarFotoPerfilScreen = ({route, navigation}) => {
    ...
    const [foto, setFoto] = React.useState(route.params.ImgPerfil);      
    const [fotoOpener, setFotoOpener] = React.useState(false); 
    ...
    rendFoto = () =>{
      console.log(foto)
        if(foto){
          const externalLink = `https://convey.cl/partner/fotoPerfil/${foto}`;
          const isLocalUri = foto.startsWith('file://') || foto.startsWith('content://');
          const isExternalFile = foto.endsWith('.jpeg') || foto.endsWith('.jpg') || foto.endsWith('.png');
          return(
            <View style={{position:'relative',}}>
                ...
                source={
                  isLocalUri 
                      ? { uri: foto } // URI local tiene prioridad
                      : isExternalFile 
    ...
    React.useEffect(() => {
      ...
      const AddFoto = () =>{
        ...
        fetch(`${ServidorExport}/partner/CrearFoto.php`, {
          method: 'POST',
          headers: {
      ...
      const EditFoto = () => {
          ...
          fetch(`${ServidorExport}/partner/EditarFoto.php`, {
            method: 'POST',
            headers: {
              Accept: 'application/json',
              'Content-Type': 'multipart/form-data',
            ...
            .then((response) => {
              if (!response.ok) {
                ...
                response.json().then((responser) => {
                  setActivityShow((prev) => false);
                  if (responser === 'exito') {
                    ModalOptions('Exito', 'Foto actualizada', 'Los cambios se han guardado exitosamente', passBack);
                  } else {
                    ModalOptions('Error', 'Ooops!', 'Hubo un problema, intenta nuevamente', 'close');
                  }
                });
            ...
            <ScrollView style={{flex:1}} keyboardShouldPersistTaps='always'>
              ...
              <View style={styles.header}>
               ...
               <Text style={styles.TextHeader}>Foto de perfil</Text>
                ...
                <TouchableOpacity style={{width:'100%', backgroundColor:'#E6E6FA', borderRadius:10, padding:10, justifyContent:'center', alignItems:'center', marginBottom:15, marginTop:15}} onPress={() => {openSelectMethodImage('Foto')}}>
                ...
                {rendFoto()}
                    ...
                    <TouchableOpacity style={{width:'100%', height:'100%', justifyContent:'center', alignItems:'center'}} activeOpacity={1} onPress={() => EditFoto()}>
                        <Text style={{ fontWeight: 'bold', color: '#000' }}>Editar</Text>
                    </TouchableOpacity>
                    </LinearGradient>
                ) : (
                    <LinearGradient style={{width:'100%', height:50, borderRadius:10, justifyContent:'center', alignItems:'center'}} colors={['#E6F14B', '#E6F14B', '#C3FE70']} start={{ x: 1, y: 0 }} end={{ x: 0, y: 0 }}>
                    <TouchableOpacity style={{width:'100%', height:'100%', justifyContent:'center', alignItems:'center'}} activeOpacity={1} onPress={() => AddFoto()}>
...
export default EditarFotoPerfilScreen;
