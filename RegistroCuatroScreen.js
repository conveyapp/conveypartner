...
const RegistroCuatroScreen = ({ route, navigation }) => {
  ...
  const determineImageRotation = (width, height) => {
    // Para cédulas, siempre usar orientación horizontal
    // Si la imagen es más alta que ancha (portrait), rotar -90 grados
    if (height > width) {
  ...
  const takePicture = async () => {
    ...
    if (cameraRef.current) {
      ...
      // Determinar la rotación correcta basada en las dimensiones (igual que licencia)
  ...
  NextPageCuatroScreen = () => {
    ...
    if (image == '' || image == null) {
      setLoad(false)
      ModalOptions('Error', 'Error Anverso', 'Debes subir una imagen del anverso de tu C.I', 'close');
      ...
      setLoad(false)
      navigation.navigate('RegistroCinco', { FirmaUri: firma, Nombre: nombre, Numero: numero, Rut: rut, Direccion: direccion, FechaNacimiento: fechaNacimiento, Correo: correo, Clave: clave, ImgPerfil: imgPerfil, CAnverso: image, CReverso: imageTwo });
    }
  ...
  const pickImage = async () => {
    ...
    if (!result.canceled) {
      ...
      // if(result.assets[0].height != 400 || result.assets[0].width != 400){
  ...
  const pickImageTwo = async () => {
    ...
    if (!result.canceled) {
      ...
      setBase64Two(result.assets[0].base64);
      //}
    }
      ...
      return (
        ...
        <Image
          ...
          style={{
            ...
            marginTop: 10,
            marginBottom: 10,
            // Rotación por estado, igual que licencia
        ...
        <View style={{ flex: 1 }}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <View style={styles.containerAdornos}>
              ...
              <View style={{ width: '100%', height: '100%', alignItems: 'center', justifyContent: 'center', marginTop: 0, paddingHorizontal: 15, }}>
                <Text style={[styles.TextHeader, { marginBottom: 15, }]}>Cédula de identidad</Text>
                ...
                <LinearGradient style={{ width: '100%', height: 50, borderRadius: 10, justifyContent: 'center', alignItems: 'center' }} colors={['#E6F14B', '#E6F14B', '#C3FE70']} start={{ x: 1, y: 0 }} end={{ x: 0, y: 0 }}>
                  ...
                  <TouchableOpacity style={{ width: '100%', height: '100%', justifyContent: 'center', alignItems: 'center' }} activeOpacity={1} onPress={() => NextPageCuatroScreen()}>
...
export default RegistroCuatroScreen;
